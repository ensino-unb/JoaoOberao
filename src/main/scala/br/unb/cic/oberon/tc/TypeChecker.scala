...
class ExpressionTypeVisitor(val typeChecker: TypeChecker) extends OberonVisitorAdapter {
  ...
  override def visit(exp: Expression): Option[Type] = exp match {
    ...
    case LongIntValue(_) => Some(LongIntType)
